...
**Version 1.0**  
**Built with:** Python, Django, Django Channels, Daphne, Exotel AgentStream, OpenAI Realtime, PostgreSQL/SQLite
...
- Connect the live audio stream to OpenAI Realtime.
...
```text
Vardha_Voice_Connect_Django_Exotel_v1.0/
│
...
│   ├── consumers.py                # Exotel ↔ OpenAI live audio bridge
│   ├── models.py                   # Calls and Knowledge Base models
│   ├── services.py                 # Exotel, OpenAI and summary services
...
The Exotel AgentStream/OpenAI reference implementation also requires an externally reachable WSS endpoint and uses `sample-rate=8000` for telephony audio. It explicitly keeps the debug test tone disabled by default.
...
The voice bridge is designed around the current Exotel AgentStream/OpenAI pattern:

- Exotel telephony audio: PCM16, 8 kHz.
- OpenAI Realtime audio: PCM16, 24 kHz.
- Audio is resampled locally in both directions.
- OpenAI output is buffered before it is sent to Exotel.
...
- The OpenAI connection starts only after Exotel's `start` event.
- No test tone is generated by this application.
- The greeting is sent only after both the Exotel stream and OpenAI session are ready.
...
If `session.updated` never appears, check the OpenAI key, model and Realtime access.
...
- OpenAI Realtime access and API key.
